...
- **Permissions** (header button) — who may run which tools, assigned from a list of the people who have actually called (see *Per-user identity*)
...
The **Statistics** button in the header opens a report across all instances: calls within a chosen window (24 h / 7 / 30 / 90 days), calls in total, when each was last used, and a bar per day. Click a row to break it down **per function** — which is where it gets interesting: a tool whose ten functions include three nobody ever calls is carrying schemas through every request for nothing.
...
> Function schemas are derived from the tool source, including uploaded OpenWebUI JSONs. See [Upgrading](upgrading.md) for existing installations.
...
Each instance runs in a separate process using a named Python environment under `runtime/venvs/<name>/`. Instances using the same name share packages; by default they all use `default`. Assign different environment names when tools need incompatible dependencies. The manager uses its own `.venv`. Python virtual environments separate packages, not filesystem or operating-system permissions.
...
For older environments and one-time migrations, see [Upgrading](upgrading.md).
...
Switch **File storage** on in the **Edit** dialog. The instance then gets `content/<id>/`, and the path reaches the tool three ways, so a tool written for OpenWebUI usually runs unchanged:
...
Backups include tool source files as well as instance configurations. A configuration alone cannot restore a tool whose source is missing.
...
**Credentials are a checkbox.** With them the archive restores a working server and *is* a collection of credentials — the MCP, read and agent tokens, the user-JWT secret, the file key, and every API key in the instance valves. The filename then carries `-with-secrets` and the dialog says what that means, because a backup whose contents nobody can tell by looking is one that ends up in the wrong folder. Without them known credential fields are redacted; review the archive before sharing it. After a redacted restore, no instance holding an API key will run until the keys are entered again, and old download links stay broken because the file key is a different one.
...
Restore merges missing entries into the target. Review the restore report for skipped entries, reassigned ports and credentials that need to be entered again.
...
### Users & permissions (the Permissions dialog)

Rights are assigned in the dashboard, not in a text editor. The **Permissions** button opens a list of everyone the runners have actually seen — recorded on their first verified call, so a new colleague appears the moment they use anything — plus everyone a rule already names.
...
When auth is active:
- The web UI opens in guest mode without a saved login. Click **Login** to authenticate. The password is verified against a protected endpoint (`GET /api/auth-check`) — wrong passwords are rejected immediately.
...
1. Upload `examples/mcp-manager-control.json` using **Upload JSON** to create and install the `mcp_manager_control` instance, then start it.
2. In your client add it as an MCP server at `http://<host>:<port>/mcp`, with the Bearer token if MCP auth is enabled.

**Available tools (33):** `list_instances`, `get_instance`, `start_instance`, `stop_instance`, `restart_instance`, `reinstall_instance`, `get_install_log`, `get_runtime_log`, `get_instance_config`, `get_settings`, `list_categories`, `list_agent_identities`, `check_backup`, `get_instance_specs`, `get_usage_stats`, `get_system_stats`, `get_tool_code`, `save_tool_code`, `validate_tool_code`, `export_tool`, `delete_instance`, `restart_manager`, `get_tool_template`, `upload_tool`, `create_tool`, `update_instance_category`, `update_instance_values`, `update_instance_lifecycle`, `update_instance_dependencies`, `update_instance_venv`, `list_content`, `read_content`, `delete_content`.
...
**Security — enforced server-side, granular per action.** The control instance carries one `allow_*` Valve per action (`allow_delete`, `allow_create_tool`, `allow_restart`, `allow_manager_restart`, …). Most actions, including code edits and process start/stop, are enabled by default. Only `allow_delete`, `allow_manager_restart` and `allow_content_delete` default to `false`; enable them explicitly through their Valve in **Edit → Values** (the spawner restarts the instance itself, so the change takes effect right away) — so an agent can never do more than you've allowed. Write operations also need a credential, supplied to the control instance via the `auth_token` Valve; `manager_url` points it at the spawner API. Give it the [agent token](#api-tokens) rather than the password — it does the same job, is revocable on its own, and cannot read out the other credentials or set a new password. With every write Valve off, the read token is enough and the reach shrinks to reading. The global edit mode (`--no-edit` / `--no-code-edit`) still applies on top.

---

## One connection for all tools (MCP tool router)

Connecting a client directly to many instances can expose all their function schemas up front, even when a request needs only one function.

The **tool router** in `examples/mcp-tool-router.json` (instance id `mcp_tool_router`) replaces that with **one** connection exposing **three** meta-tools:

| Tool | Purpose |
|---|---|
| `find_tools(query_words=[], category="", mode="or")` | The directory. **Without arguments the allowed running catalog, up to `max_results` (default 60)**: one line per function, `instance.action — what it does`, no schemas |
...
The model finds a tool, fetches its schema, then calls it. Schemas are loaded on demand; the context savings depend on the installed tools and client.


**Setup**

1. Upload `examples/mcp-tool-router.json` and start the instance.
2. In **Edit → Values**, set `manager_url` to the manager address (default `http://127.0.0.1:7860`). For a protected manager, set `auth_token` to its [read token](#api-tokens). Independently, set `mcp_token` if the target MCP endpoints require a Bearer token. Read-only manager access does not prevent calls to tools that write data.
...
**Client approvals.** Configure tool approvals in the MCP client you use. Permission to call the router's `call_tool` can cover every tool it exposes, including tools that write data. Use the router's `deny_instances` and `allow_categories` to limit its catalog; identity-aware targets enforce their own access rules. Client configuration syntax is client- and version-specific.
...
sudo nano /etc/owui-mcp-spawner.env   # set MCP_MANAGER_PASSWORD; configure the MCP token in Settings
```
...
Run tests in a disposable checkout with separate runtime state. Contributor guidance and isolation requirements are documented in [tests/README.md](../tests/README.md).
